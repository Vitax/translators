...
function writeField(field, value, isMacro) {
	...
	if (!isMacro && !(field == "url" || field == "doi" || field == "file" || field == "lccn" || field == "expressionsofconcern" || field == "retraction" )) {
...
async function doExport() {
	...
	while (item = Zotero.nextItem()) {
		...
		if (type == "phdthesis") {
			...
			var thesisType = item.type && item.type.toLowerCase()
				.replace(/[\s.]+|thesis|unpublished/g, '');
		...
		if (Zotero.Utilities.isRetracted(item) && Zotero.getOption("exportRetractions")) {
			await Zotero.Utilities.getRetractionInformation(item)
				.then(retractionData => {
					let retractionString = "";
					if (retractionData.doi || retractionData.pmid) {
						if (retractionData.doi) {
							retractionString = retractionString.concat('https://doi.org/' + retractionData.doi);
						}
						else {
							retractionString = retractionString.concat('https://www.ncbi.nlm.nih.gov/pubmed/' + retractionData.pmid);
						}
					}

					writeField('retraction', retractionString);
				});
		}

		if (Zotero.Utilities.hasExpressionOfConcern(item) && Zotero.getOption("exportExpressionsOfConcern")) {
			await Zotero.Utilities.getExpressionOfConcernInformation(item)
				.then(eoc => {
					let eocString = "";
					let eocData = eoc.data;

					for (let i = 0; i < eocData.links.length; i++) {
						if (i != 0) {
							eocString = eocString.concat(",{", eocData.links[i], "}\n");
						}
						else {
							if (eocData.links.length > 1) {
								eocString = eocString.concat("{", eocData.links[i], "}\n");
							}
							else {
								eocString = eocString.concat(eocData.links[i]);
					...
					writeField("expressionsofconcern", eocString)
				});
			Zotero.write("\n}");
		}
	}
